...
limpiar <- function(){
  objects_to_remove <- ls()[sapply(ls(), function(x) !is.function(get(x)))]
...
````{r}
create_pathways <- function(dataframe, param){
  output_dir <- paste0(param, "/rutas")
  if (!dir.exists(output_dir)) {
    dir.create(output_dir)
}
  gene_data <- c(na.omit(dataframe$down$Datos$logFC), na.omit(dataframe$up$Datos$logFC) )
  names(gene_data) <- c(na.omit(dataframe$down$Datos$ENTREZID), na.omit(dataframe$up$Datos$ENTREZID))
  
  pathways <- c(na.omit(dataframe$down$KEGG$ID[dataframe$down$KEGG$p.adjust<0.05]), na.omit(dataframes_bayes$up$KEGG$ID[dataframe$up$KEGG$p.adjust<0.05]))

  for (pat in pathways){
    output_file <- file.path(output_dir, paste0(pat,".png"))
    tryCatch({
    ...
    temp_file <- paste0(pat, ".temp.png")
    if (file.exists(temp_file)) {
      file.rename(temp_file, output_file)
    } else {
      message("Archivo temporal no encontrado para ", pat)
    }
  }, error = function(e) {
    message("Error al procesar la ruta ", pat, ": ", e$message)
  })
  }
